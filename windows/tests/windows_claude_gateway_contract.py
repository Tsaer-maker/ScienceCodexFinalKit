...
def verify_template(path: Path) -> None:
    ...
    for mode, profile in value["profiles"].items():
        ...
        if mode == "codex":
            ...
            assert profile["name"] == "FinalKit Windows Codex Login"
        else:
...
def verify_conversion(module) -> None:
    ...
    kimi = module.apply_provider_reasoning({}, "kimi", "low")
    assert kimi == {"thinking": {"type": "enabled"}, "reasoning_effort": "low"}
...
class FakeRefreshOpener:
    ...
    def open(self, request, timeout: int):
        ...
        assert "refresh_token=offline-refresh-old" in form
        assert "client_id=app_EMoamEEZ73f0CkXaXp7hrann" in form
        self.calls += 1
...
def verify_codex_auth(module) -> None:
    with tempfile.TemporaryDirectory(prefix="finalkit-windows-codex-auth-") as raw_dir:
        ...
        old_access = jwt({"exp": int(time.time()) + 3600})
        new_access = jwt({"exp": int(time.time()) + 3600})
        ...
        opener = FakeRefreshOpener(
            {
                "access_token": new_access,
                "refresh_token": "offline-refresh-new",
            }
        )
        store = module.CodexAuthStore(auth_path, opener=opener)
        ...
        assert refreshed["tokens"]["refresh_token"] == "offline-refresh-new"
        assert refreshed["preserved_owner_field"] == "keep-me"
        assert refreshed["OPENAI_API_KEY"] is None
...
def verify_process(gateway_path: Path) -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="finalkit-windows-claude-") as raw_dir:
        ...
        config = {
            ...
            "profile_name": "FinalKit Windows Codex Login",
            "host": "127.0.0.1",
            "port": port,
        ...
        assert health["routes"]["sonnet"] == {"model": "gpt-test-sonnet", "reasoning": "high"}
...
def main() -> int:
    ...
    print(
        ...
        "codex-routes=opus+sonnet+haiku reasoning=per-route+listed "
        "dpapi=3 codex_auth=official+refresh loopback=verified responses=tools+stream"
    )
    return 0
