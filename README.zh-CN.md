...
该命令临时注入本地 endpoint，不把真实 API key 交给 Claude Code 进程，也不要求 Claude Science 已启动或已登录。若需要 Claude Science 工作台，仍须在 Science 页面完成其支持的 Claude 账号登录；provider API key 和 ChatGPT/Codex OAuth 只负责本地 gateway，不能替代 Science 账号。
...
- Science 0.1.27 只由 FinalKit 注入本地 `ANTHROPIC_BASE_URL`，Science 账号会话仍由其官方登录流程拥有；FinalKit 不伪造 Claude OAuth、订阅或服务端权限。旧 FinalKit 虚拟 token 只有在完整识别后才会被移除，未知或真实 Science 凭据原样保留；
...
- Claude 账号与 Console/API 账号彼此独立：[Can I have a Claude account and a Console account?](https://support.claude.com/en/articles/8987223-can-i-have-a-claude-account-and-a-console-account)
- Claude 官方登录与第三方工具认证边界：[Log in to your Claude account](https://support.claude.com/en/articles/13189465-log-in-to-your-claude-account)
